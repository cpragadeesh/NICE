...
void parse(char *line, struct command *commandData) {  
  ...
  for(j = commandNameLength; j < strlen(line); j++) {
    ...
    if(line[j] == '"') {
      j++;
      commandData->numberOfValues++;
      commandData->val[commandData->numberOfValues] = malloc(MEMLIMIT);
      int i = 0;
      while(line[j] != '"') {
      	commandData->val[commandData->numberOfValues][i] = line[j];
      	j++;
      	i++;
      }
    }
  ...
  for(j = 0; j <= commandData->numberOfValues; j++)
...
void destroy_command(struct command *commandData) {

  if(commandData) {
  	if(commandData->commandName) {
      free(commandData->commandName);
  	}
  }

  for(int i = 1; i < commandData->argLength; ++i) {
    free(commandData->arg[i]);
  }

  for(int i = 0; i < commandData->numberOfValues; ++i) {
    free(commandData->val[i]);
  }
}
...
void destroy_strings() {
	...
	for(int i = 0; i < MEMLIMIT; ++i) {
		for(int j = 0; j < HASHBUCKETS; ++j) {
			if(stringVariable[i][j]) {
				free(stringVariable[i][j]);
			}
		}
	}
